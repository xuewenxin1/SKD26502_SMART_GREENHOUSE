...
static const char MAGIC_NUMBER_LEGACY[4] = {'C','F','1',0};
...
static const Config_t default_system_config = {
    ...
    .working_mode = WorkingMode_Manual,
    .last_opening_percentage = 0,
    .temp_vent_upper_limit = 250,
    .temp_vent_lower_limit = 200,
    .motor_turn_seconds = MOTOR_TURN_SECONDS_DEFAULT,
...
static void clamp_last_position(Config_t &config){
    ...
    const int max_cnt = stroke_sec * 10 + stroke_sec;
    int cnt = config.last_motor_timer_cnt;
    if ( (cnt < 0) || (cnt > max_cnt) ){
        cnt = (int)((double)op * 10.0 * (double)stroke_sec / 100.0 + 0.5);
    }
