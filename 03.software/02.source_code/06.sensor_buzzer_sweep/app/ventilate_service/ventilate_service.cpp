...
static void try_persist_stopped_opening(void){
    ...
    if ( timer_cnt > 2000000000LL ){
        timer_cnt = 2000000000LL;
    }
    saved.last_opening_percentage = value;
    saved.last_motor_timer_cnt = (int)timer_cnt;
    ConfigService::set_config(saved);
    ConfigService::store();
    persisted_opening = value;
    pending_persist_opening = -1;
    LOG_INFO("Save opening %d percent, timer %d (0.1s) after motor stop.",
             value, saved.last_motor_timer_cnt);
}
