...
void process_act_msg(uint8_t act, uint16_t param){
    ...
    switch ( act ){
        ...
        case 1: {
            /* param 为精确目标圈数（不×10）；按 C1 换成开度%，行程时间仍是 圈数×C2. */
            Config_t cfg;
            ...
            LOG_INFO("MQTT set turns %d -> opening %d percent (C1=%d C2=%d).",
                     turns, opening, stroke, cfg.motor_turn_seconds);
            if ( VentilateService::set_opening_percentage(opening) == false ){
                LOG_WARN("MQTT set opening rejected (ALIGN/calibrate).");
            }
            break;
        }
        case 2: {
    ...
    bool has_ext_fields)
{
    ...
    LOG_INFO("MQTT config update done. mode %d C1=%d C2=%d P5=%d P6=%d.",
