...
void process_config_msg(
    ...
    uint16_t temp_return_diff_negative, uint16_t temp_compensation_value, uint8_t device_valid)
{
...
void process_iot_down(const char *topic, unsigned char *buffer, unsigned int size){
    ...
    if ( strncmp(pack_type,"act",3) == 0 ){
        ...
        process_config_msg(mode,temp_alarm_upper_limit,temp_alarm_lower_limit,target_temp,temp_return_diff_positive,temp_return_diff_negative,temp_compensation_value,device_valid);
