...
void process_act_msg(uint8_t act, uint16_t param){
    ...
    switch ( act ){
        ...
        case 1: {
            /* param 为精确目标圈数（不×10）；内部按 C1 算小数开度，避免整除截断提前停. */
            Config_t cfg;
...
void process_config_msg(
    ...
    uint16_t temp_return_diff_negative, int16_t temp_compensation_value, uint8_t device_valid,
    ...
    bool has_ext_fields)
{
    ...
    config.device_valid = device_valid;
    if ( has_ext_fields ){
...
void process_time_and_id(uint8_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t id[6]){
    ...
    VentilateService::set_clock(hour, minute, true);
}
...
void process_iot_down(const char *topic, unsigned char *buffer, unsigned int size){
    ...
    if ( strncmp(pack_type,"act",3) == 0 ){
        ...
        /* 旧包 15；扩包≥22 含 C1/C2/P5/P6；≥24 再含温度校准(P4). */
        if ( size < 15 ){
        ...
        uint8_t device_valid = buffer[13];
        ...
        process_config_msg(mode,temp_alarm_upper_limit,temp_alarm_lower_limit,target_temp,
                           temp_return_diff_positive,temp_return_diff_negative,temp_compensation_value,device_valid,
